    ...
    <section class="card">
      <div class="row" style="justify-content: space-between; margin-bottom: 16px;">
        <div>
          ...
          <div class="card-sub">${translateWithFallback("library.subtitle", "Your persisted local sessions and project history.")}</div>
        </div>
      ...
      ${
        props.loading && props.sessions.length === 0
          ? html`<div class="muted">${translateWithFallback("common.loading", "Loading sessions...")}</div>`
          : nothing
      }

      ${
        !props.loading && props.sessions.length === 0
          ? html`<div class="muted">${translateWithFallback("library.empty", "No local sessions found. Start a chat to create one.")}</div>`
          : nothing
      }
      ...
      <div class="row" style="justify-content: space-between; align-items: flex-start;">
        <div style="flex: 1; min-width: 0;">
          <div class="row" style="gap: 8px; align-items: center;">
            ...
            ${
              isActive
                ? html`<span class="pill ok" style="font-size: 10px; padding: 2px 6px;">${translateWithFallback("common.active", "Active")}</span>`
                : nothing
            }
          </div>
