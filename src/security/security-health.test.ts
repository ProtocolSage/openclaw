...
function applyDefaultMocks(): void {
  ...
  } as ReturnType<typeof monitorRunnerMod.getMonitorRunner>);
  ...
  } as ReturnType<typeof securityEventsMod.getSecurityEventsManager>);
  ...
  } as ReturnType<typeof sessionMonitoringMod.getSessionRiskMonitor>);
}
...
function makeCredential(name: string): credentialVaultMod.CredentialEntry {
  return { name, scope: "provider", createdAt: 0, updatedAt: 0, hashPrefix: "abc" };
}
      ...
      mockQuerySecurityEvents.mockImplementation(
        (filters?: securityEventsMod.SecurityEventQueryFilters) => {
          ...
          return [
            ...
            makeSecurityEvent("credential_access", "warn"), // should not count
