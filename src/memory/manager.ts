...
export class MemoryIndexManager implements MemorySearchManager {
  ...
  async search(query: string, opts?: MemorySearchOptions): Promise<MemorySearchResult[]> {
    ...
    if (!hybrid.enabled) {
      const riskAdjusted = await this.applySearchRiskPolicy(vectorResults, opts?.allowUntrusted);
      return riskAdjusted.filter((entry) => entry.score >= minScore).slice(0, maxResults);
    }
    ...
    const riskAdjusted = await this.applySearchRiskPolicy(merged, opts?.allowUntrusted);
    return riskAdjusted.filter((entry) => entry.score >= minScore).slice(0, maxResults);
  }
  ...
  private readRiskMetadataForEntry(
    entry: Pick<MemorySearchResult, "path" | "source">,
    cache: Map<string, MemoryRiskMetadata | null>,
  ): MemoryRiskMetadata | null {
    ...
    const row = this.db
      .prepare(
        `SELECT risk_level, risk_score, risk_classes, risk_patterns, risk_encoded_matches
         FROM files WHERE path = ? AND source = ? LIMIT 1`,
      )
      .get(entry.path, entry.source) as
      | {
          risk_level?: unknown;
          risk_score?: unknown;
    ...
    const parsed = parseRiskMetadataRow(row);
  ...
  private async applySearchRiskPolicy(
    ...
    const riskCache = new Map<string, MemoryRiskMetadata | null>();
    const filtered: MemorySearchResult[] = [];
    for (const entry of results) {
      const risk = this.readRiskMetadataForEntry(entry, riskCache);
  ...
  private async indexFile(
    ...
    this.db
      .prepare(
        `INSERT INTO files (
           path, source, hash, mtime, size, risk_level, risk_score, risk_classes, risk_patterns, risk_encoded_matches
         ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
         ON CONFLICT(path) DO UPDATE SET
           ...
           risk_encoded_matches=excluded.risk_encoded_matches`,
      )
      .run(
        entry.path,
        options.source,
        ...
        risk.riskLevel,
        risk.score,
        JSON.stringify(risk.classesMatched),
        JSON.stringify(risk.patternsTop),
        risk.encodedMatches,
      );
  }
}
