...
export class QmdMemoryManager implements MemorySearchManager {
  ...
  >();
  private readonly riskMetadataCache = new Map<string, MemoryRiskMetadata>();
  ...
  async search(query: string, opts?: MemorySearchOptions): Promise<MemorySearchResult[]> {
    ...
    const riskAdjusted = await this.applySearchRiskPolicy(
      results.slice(0, limit),
      opts?.allowUntrusted,
    );
    return this.clampResultsByInjectedChars(
      riskAdjusted.filter((entry) => entry.score >= (opts?.minScore ?? 0)),
    );
  }
  ...
  async readFile(params: {
    ...
    if (inspection.riskLevel === "critical" && !params.allowUntrusted) {
      log.warn("memory read blocked: critical prompt-injection patterns detected", {
        ...
        classesMatched: risk.classesMatched,
    ...
    const warnings =
      inspection.riskLevel === "medium" || inspection.riskLevel === "high"
        ...
        : inspection.riskLevel === "critical"
          ? [buildRiskWarning({ risk, prefix: "CRITICAL" })]
          : undefined;
    if (!params.from && !params.lines) {
      return { text: content, path: relPath, warnings };
    }
    const lines = content.split("\n");
    const start = Math.max(1, params.from ?? 1);
    const count = Math.max(1, params.lines ?? lines.length);
    const slice = lines.slice(start - 1, start - 1 + count);
    return { text: slice.join("\n"), path: relPath, warnings };
  }
  ...
  private getRiskCacheKey(entry: Pick<MemorySearchResult, "source" | "path">): string {
    return `${entry.source}:${entry.path}`;
  }

  private getRiskSidecarPath(docPath: string): string {
    return `${docPath}.meta.json`;
  }

  private async writeRiskSidecar(docPath: string, risk: MemoryRiskMetadata): Promise<void> {
    const sidecarPath = this.getRiskSidecarPath(docPath);
  ...
  private async readRiskSidecar(docPath: string): Promise<MemoryRiskMetadata | null> {
    const sidecarPath = this.getRiskSidecarPath(docPath);
    let raw = "";
    try {
    ...
    try {
      ...
      return coerceRiskMetadata(parsed);
    } catch {
      return null;
    }
  }

  private async resolveResultRisk(entry: MemorySearchResult): Promise<MemoryRiskMetadata> {
    const cacheKey = this.getRiskCacheKey(entry);
    const cached = this.riskMetadataCache.get(cacheKey);
    if (cached) {
      return cached;
    }

    let risk: MemoryRiskMetadata;
    let absPath: string | undefined;
    try {
      absPath = this.resolveReadPath(entry.path);
    } catch {
      absPath = undefined;
    }

    if (absPath) {
      const sidecar = await this.readRiskSidecar(absPath);
      if (sidecar) {
        this.riskMetadataCache.set(cacheKey, sidecar);
        return sidecar;
      }
    }
  ...
  private async applySearchRiskPolicy(
    results: MemorySearchResult[],
    allowUntrusted = false,
  ): Promise<MemorySearchResult[]> {
    const filtered: MemorySearchResult[] = [];
    for (const entry of results) {
      const risk = await this.resolveResultRisk(entry);
      if (risk.riskLevel === "critical" && !allowUntrusted) {
        continue;
      }
      ...
      const warningPrefix = risk.riskLevel === "critical" ? "CRITICAL" : "WARNING";
      filtered.push({
        ...entry,
        score: entry.score * riskScoreMultiplier(risk.riskLevel),
        riskLevel: risk.riskLevel,
        riskClasses: risk.classesMatched,
        warnings: [buildRiskWarning({ risk, prefix: warningPrefix })],
      });
    }
